  ...
  get: {
    all: (req, res) => {
      ...
      contactModel
        ...
        .then((contacts) => {
          // const arr = contacts.filter(res=>res.)
          const result = contacts.contacts.filter(
            (res) => res.hotelId.resoltId.name === 'Albena'
          );

          res.status(200).json({ status: true, contacts, result });
        })
        .catch((err) => {
