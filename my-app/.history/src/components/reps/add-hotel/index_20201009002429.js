...
const Test = (props) => {
  ...
  const rep = 0;
  useEffect(() => {
    Promise.all([hotelsService.listAll(), repsService.getHotelsByRep(repId, token)])
      .then(([allHotels, repsHotel]) => {
        setAllHotels(allHotels.hotels);
        setHotelsRep(repsHotel.hotels);
      })
      .then(([allHotels, repsHotel]) => checked(allHotels.hotels, repsHotel.hotels));

    //   .then(checked());
    // checked();
  }, []);

  function checked(hr, ha) {
