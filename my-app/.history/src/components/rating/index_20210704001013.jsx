...
const UserRating = () => {
  ...
  useEffect(() => {
    ...
    contactService
      .reservationInfo(cryptedResId)
      .then((res) => {
        // if (!!isSubscribed) {
        // throw new Error('vasko e pi4!');
        console.log({ res });
        if (!res.contact) {
        ...
        setRating({ ...rating, resId: res?.contact?._id, hotelId: res?.contact?.hotelId?._id.toString() });
        // return;
        // }
      })
      .catch(alert);
  ...
  return (
    <div className={ratingStyles.ratingWrapper}>
      ...
      {resInfo === 'empty' ? (
        ...
        <div>
          <section className={ratingStyles.rating}>
            <div>
            ...
            <div>reservetion number: {rating?.resId}</div>
            <div>
