...
const Test = (props) => {
  ...
  useEffect(() => {
    ...
    Promise.all([hotelsService.listAll(), repsService.getHotelsByRep(repId, token), repsService.details(repId)]).then(
      ([allHotels, repsHotel, repsInfo]) => {
  ...
  return (
    <React.Fragment>
      <h2>Rep hotels assignment for {repName}</h2>
