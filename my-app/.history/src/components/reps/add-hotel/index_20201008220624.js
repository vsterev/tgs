...
const Test = (props) => {
  ...
  console.log(hotelsRep.hotels);
  useEffect(() => {
    hotelsService.listAll().then((allHotels) => setAllHotels(allHotels));
    repsService.getHotelsByRep(repId, token).then((htl) => setHotelsRep(htl.hotel));
  }, []);
  const modifiedHotelsRep = hotelsRep.hotels.map((a) => a._id);
