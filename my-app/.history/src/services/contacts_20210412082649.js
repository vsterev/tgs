const ContactsService = {
  ...
  reservationInfo: (resId) => {
    return fetch(`http://localhost:4000/get-reservation/${resId}`, {
      // body: JSON.stringify(data),
      method: 'GET',
  ...
  welcomeSendMessage: (date, token) => {
    ...
    return fetch(`http://localhost:4000/linkMobility/checkIn-message/${date}`, {
      method: 'GET',
      headers: { 'Content-type': 'application/json', Authorization: token },
    })
      .then((res) => res.json())
  ...
  goodByeSendMessage: (date, token) => {
    ...
    return fetch(`http://localhost:4000/linkMobility/checkOut-message/${date}`, {
      method: 'GET',
      headers: { 'Content-type': 'application/json', Authorization: token },
    })
      .then((res) => res.json())
