...
const Test = (props) => {
  ...
  const [repName, setRepName] = useState({ firstName: null, secondName: null, pr: 'vasko' });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (allHotels.status & repsHotel.status) {
        ...
        setRepName({ firstName, familyName, ...repName });
        return;
      }
  ...
  return (
    <React.Fragment>
      <h2>
        hotels assignment to rep - {repName.firstName} {repName.familyName} {repName.pr}
      </h2>
