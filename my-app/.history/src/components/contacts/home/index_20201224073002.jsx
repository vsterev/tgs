  ...
  const submitHandler = (e) => {
    ...
    contactsService
      ...
      .then((result) => {
        ...
        const hotels = result.map((rs) => rs.hotelId.resortId.name);
        console.log(hotels);
        console.log([...new Set(hotels)]);
        // setHotelsArr(uniqueValFromArray(hotels, 'hotelId'));
      })
      ...
      <form onSubmit={submitHandler}>
        ...
        {hotelsArr.length !== 0 && (
          <label htmlFor="hotels">
            hotel
            <select name="hotelId" id="hotels" onChange={(e) => console.log(e.target.value)}>
              {hotelsArr.map((hl, i) => {
                return (
                  <option key={i} value={hl}>
                    {hl}
                  </option>
                );
              })}
