...
const SearchRep = ({ token, setReps, setErr, reps }) => {
  ...
  const searchRep = (e) => {
    ...
    const searched = [...reps].filter((a) => {
      if (a.firstName.toLowerCase().includes(nameRep.toLowerCase()) & hotelIncludes(a, hotel)) {
        return true;
      } else if (a.familyName.toLowerCase().includes(nameRep.toLowerCase()) & hotelIncludes(a, hotel)) {
        return true;
      } else if (
        (a.firstName + ' ' + a.familyName).toLowerCase().includes(nameRep.toLowerCase()) & hotelIncludes(a, hotel)
      ) {
        return true;
      }
    });
