...
const ArrivalByDate = () => {
  ...
  const sendMessageHandler = () => {
    ContactsService.welcomeSendMessage(date, token)
      .then((r) => {
        console.log('r', r);
        setSendedInfo(r);
        setMessagInfo(`sended to ${dataSended} contacts, errors in ${noPhnones.length + noRepsAdded.length} contacts`);
      })
      .then(() => {
  ...
  return (
    <div className={styles.arrivalwrapper}>
      ...
      {Object.keys(sendedInfo).length > 0 && JSON.stringify(sendedInfo)}
    </div>
  );
};
